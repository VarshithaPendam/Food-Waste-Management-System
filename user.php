<?php
// Database connection parameters
$servername = 'localhost';
$username = 'root';
$password = '';
$database = 'reg';

// Establish database connection
$conn = new mysqli($servername, $username, $password, $database);

// Check connection
if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
}

// Query to fetch data
$sql = "SELECT name, contact, tof, qof, addr FROM donor"; // Modify this query based on your table structure
$result = $conn->query($sql);

// Fetch data
$data = array();
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $data[] = $row;
    }
}
$conn->close();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>India Eats</title>
    <script src="script.js" defer></script>
    <link rel="stylesheet" href="/IndiaEats/CSS/user.css">
</head>
<body>
<header>
        
        <h1>India Eats</h1>
        <p>Your solution for food waste management</p>
        <div class="dropdown">
            <img src="data:image/jpeg;base64," alt="User Icon">
            <div class="dropdown-content">
                <a href="http://localhost/fwms/update.php/">Update Profile</a>
                 <a href="http://localhost/IndiaEats/index.php/">Logout</a>
                 <a href="http://localhost/IndiaEats/chat.php/">help</a>
            </div>
        </div>
    </header>
    <nav>
        <a href="http://localhost/IndiaEats/home.php/">Home</a>
        <a href="http://localhost/IndiaEats/about.php/">About Us</a>
        <a href="http://localhost/IndiaEats/contact.php/">Contact Us</a>
        <a href="http://localhost/IndiaEats/user.php/">Food Info</a>
    </nav>
    <main>
        <div class="container">
            <h2>Donor Data</h2>
            <div id="data-container">
                <?php if (count($data) > 0): ?>
                    <table>
                        <tr>
                            <th>Name</th>
                            <th>Contact</th>
                            <th>Type of Food</th>
                            <th>Quantity of Food</th>
                            <th>Address</th>
                            <th>Action</th>
                        </tr>
                        <?php foreach ($data as $index => $item): ?>
                            <tr id="row-<?php echo $index; ?>">
                                <td><?php echo htmlspecialchars($item['name']); ?></td>
                                <td><?php echo htmlspecialchars($item['contact']); ?></td>
                                <td><?php echo htmlspecialchars($item['tof']); ?></td>
                                <td><?php echo htmlspecialchars($item['qof']); ?></td>
                                <td><?php echo htmlspecialchars($item['addr']); ?></td>
                                <td>
                                    <button class="hide-btn" id="hide-btn-<?php echo $index; ?>" onclick="hideRow('row-<?php echo $index; ?>', 'hide-btn-<?php echo $index; ?>', 'reset-btn-<?php echo $index; ?>')">Hide</button>
                                    <button class="reset-btn" id="reset-btn-<?php echo $index; ?>" onclick="resetRow('row-<?php echo $index; ?>', 'hide-btn-<?php echo $index; ?>', 'reset-btn-<?php echo $index; ?>')" style="display:none;">Reset</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </table>
                <?php else: ?>
                    <p>No data found.</p>
                <?php endif; ?>
            </div>
        </div>
    </main>
    <footer>
        <p>&copy; 2024 All rights reserved by India Eats</p>
    </footer>

    <script>
        function hideRow(rowId, hideBtnId, resetBtnId) {
            const row = document.getElementById(rowId);
            const hideBtn = document.getElementById(hideBtnId);
            const resetBtn = document.getElementById(resetBtnId);

            row.classList.add('hidden-row');
            hideBtn.style.display = 'none';
            resetBtn.style.display = 'inline-block';
        }

        function resetRow(rowId, hideBtnId, resetBtnId) {
            const row = document.getElementById(rowId);
            const hideBtn = document.getElementById(hideBtnId);
            const resetBtn = document.getElementById(resetBtnId);

            row.classList.remove('hidden-row');
            hideBtn.style.display = 'inline-block';
            resetBtn.style.display = 'none';
        }
    </script>
</body>
</html>
